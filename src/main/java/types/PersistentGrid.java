...
public final class PersistentGrid<T> implements Grid<T> {
    ...
    public static <T> PersistentGrid<T> fromGen(IntRange xrange,
                                                IntRange yrange,
    ...
    public static <T> PersistentGrid<T> fromColumns(List<List<T>> columns) {
        return fromGen(
                new IntRange(0, columns.size() - 1),
                new IntRange(0, columns.stream().map(it -> it.size() - 1).max(Integer::compare).orElse(0)),
                pos -> {
    ...
    public static <T> PersistentGrid<T> fromRows(List<List<T>> rows) {
        return fromGen(
                new IntRange(0, rows.stream().map(it -> it.size() - 1).max(Integer::compare).orElse(0)),
                new IntRange(0, rows.size() - 1),
                pos -> {
    ...
    public PersistentGrid<T> fillWithRows(int top, int bottom, T value) {
        PersistentGrid<T> xs = bottom > 0 ?
                PersistentGrid.fromGen(new IntRange(xmin(),
                                                    xmax()
                                       ),
                                       new IntRange(ymin() - bottom,
                                                    ymin()
                                       ),
    ...
    public PersistentGrid<T> fillWithColumns(int left, int right, T value) {
        PersistentGrid<T> xs = left > 0 ?
                PersistentGrid.fromGen(new IntRange(xmin() - left,
                                                    xmin()
                                       ),
                                       new IntRange(ymin(),
                                                    ymax()
        ...
        PersistentGrid<T> ys = right > 0 ?
                PersistentGrid.fromGen(new IntRange(xmax(),
                                                    xmax() + right
                                       ),
                                       new IntRange(ymin(),
                                                    ymax()
