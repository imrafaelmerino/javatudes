...
public class Solution {
    ...
    BiFunction<Pos, Pos, Set<Pos>> lineBetween = (a, b) ->
    {
        ...
        if (a.y() == yHallway && b.y() == yHallway)
            return new IntRange(xmin, xmax)
                    .stream()
                    .mapToObj(i -> new Pos(i, yHallway))
                    .collect(Collectors.toSet());

        if (a.x() == b.x())
            return new IntRange(Math.min(a.y(), b.y()), Math.max(a.y(), b.y()))
                    .stream()
                    .mapToObj(i -> new Pos(a.x(), i))
                    .collect(Collectors.toSet());


        return Stream.of(new IntRange(xmin, xmax).stream().mapToObj(i -> new Pos(i, yHallway)),
                         new IntRange(yHallway, a.y()).stream().mapToObj(i -> new Pos(a.x(), i)),
                         new IntRange(yHallway, b.y()).stream().mapToObj(i -> new Pos(b.x(), i))
                        )
                     .flatMap(n -> n)
                     .collect(Collectors.toSet());
